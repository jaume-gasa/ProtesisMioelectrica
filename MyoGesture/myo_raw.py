...
class BT(object):
    ...
    def __init__(self, tty):
        self.ser = serial.Serial(port=tty, baudrate=9600, rtscts=1)
        self.buf = []
        self.lock = threading.Lock()
        self.handlers = []

    ## internal data-handling methods
    def recv_packet(self, timeout=None, block=True):
        t0 = time.time()
        ...
        while timeout is None or time.time() < t0 + timeout:
            ...
            ret = self.proc_byte(ord(c), block)
            if ret:
                print('ret: ', ret)
                if ret.cls is 3 and ret.cmd is 4:
                    print('payload: ', ret.payload)
                    print('Te tengo!')
                    ret = None
                    return None
                else:
    ...
    def proc_byte(self, c, block):
        if not self.buf:
            if c in [0x00, 0x80, 0x08, 0x88]:
                self.buf.append(c)
            return None
        elif len(self.buf) == 1:  #and not block
            self.buf.append(c)
            print('4 + ', (self.buf[0] & 0x07), ' + ', self.buf[1])
            if not block:
                if self.buf[1] is 3:
                    self.buf[1] = 22
            print('4 + ', (self.buf[0] & 0x07), ' + ', self.buf[1])
        ...
        print(self.packet_len, ' ', len(self.buf), ' ', self.packet_len)
        if self.packet_len and len(self.buf) == self.packet_len:
            p = Packet(self.buf)
            if p.cls is 3 and p.cmd is 4 and not block:
                print('Hola')
                self.buf = []  # Añadido
                p.cls = 4
                p.cmd = 5
    ...
    def handle_event(self, p):
        for h in self.handlers:
            if 'handle_data' in str(h) and p.cls is 3 and p.cmd is 4:
                print('IMPOSIBLE')
            else:
            #print('p: ', p)
            #print('type: ', p.typ)
...
class MyoRaw(object):
    ...
    def run(self, timeout=None):
        print(self.bt.recv_packet(timeout, block=False))
...
if __name__ == '__main__':
    ...
    p = Process(target=run_myo, args=(m,))
